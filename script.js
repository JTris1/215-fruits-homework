...
function item(idVal, nameVal, priceVal, imgLink, colorVal) {
  let that = this;
  ...
  this.div.addEventListener("click", function() {
    ...
    // that.div.classList.toggle("product");
    // that.div.classList.toggle("popUp");
...
item.prototype.popUp = function() {
  this.temp = new item(this.id + 1, this.nameVal, this.priceVal, this.imgVal, this.colorVal);
  objects.push(this.temp); 
  this.temp.div.classList.toggle("popUp");
  this.temp.div.classList.toggle("product");
  
  // Takes 2 clicks to open popup and close
  // removes the duplicates once the popup is closed
  counter++;
  if(counter === 2) {
    counter = 0;
    this.delPopUp();
...
function createElements() {
  for(let i in products) {
    let val = products[i];
    objects.push(new item(val.id, val.name, val.price, val.image, val.color));
  }
}
